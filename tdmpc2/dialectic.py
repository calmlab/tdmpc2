...
class DialecticAgent:
    def __init__(self, cfg):
        ...
        # if 'state' in cfg.obs_shape.keys():
    ...
    def _get_action_obs_dims(self):
        ...
        # self.last_action_l = torch.zeros(self.action_dim_l, device=self.device)
        # self.last_action_r = torch.zeros(self.action_dim_r, device=self.device)
...
class DialecticImitation(DialecticAgent):
    ...
    def act(self, obs, t0=False, eval_mode=False, task=None):
        ...
        state = torch.concat([obs, last_opposite_action], dim=0).unsqueeze(0)
        a = brain(state)
        if self.brain_switch:   # left brain
            self.last_action_l = a[0]
        else:  # right brain
            self.last_action_r = a[0]
    ...
    def batch_update(self, brain, inputs, action_dim):
        outputs = brain(inputs)
        mus = outputs[:, :, :action_dim]
        sigma_sqs = F.softplus(outputs[:, :, action_dim:])
        sigmas = sigma_sqs.sqrt()
        eps = torch.randn(mus.size())
        actions = mus + sigmas * eps
        probs = self.normal(actions, mus, sigma_sqs)
        log_probs = probs.log()
        entropy = self.entropy(sigma_sqs)
